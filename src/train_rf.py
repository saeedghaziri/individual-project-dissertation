...
# Random Forest uses one grid search phase. Unlike the MLP script, no second
# refinement stage is used for this model class.
RF_PARAM_GRID = {
...
# 3 x 4 x 3 x 3 x 4 = 432 parameter combinations.
...
def train_single_target(
    ...
    print(f"\n{'=' * 60}")
    print(f"[{index}/{n_total}] Training RF for {target_name}")
    print(f"{'=' * 60}")
...
def _append_result_to_csv(result: Dict[str, Any], tables_dir: str) -> None:
    ...
    csv_path = os.path.join(tables_dir, "rf_results.csv")
...
def train_all_rf(
    ...
    # Clear only the RF summary from a previous run.
    csv_path = os.path.join(tables_dir, "rf_results.csv")
